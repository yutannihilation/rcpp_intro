...
`Datetime dt(10.1)` は世界協定時 `1970-01-01 00:00:00` から `10.1` 秒経過後の時点を表す。この値を R 返すと実行者のタイムゾーンに変換された時刻として表示される。例えば日本なら日本標準時（JST）は UTC + 9時間なので、Datetime(10.0) は　`1970-01-01 09:00:10 JST` となる。
