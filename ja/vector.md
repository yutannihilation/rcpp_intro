...
64bitシステムではRのベクターの要素数は int 型の最大値よりも大きくなりうる。そのため、ベクターの要素数を変数として保持する場合には `R_xlen_t` 型を用い。
...
メンバ関数（メンバー関数とも呼ばれる）とは、個々のオブジェクトと結びついた関数である。呼び出し方が通常の関数とは少し異なっており、 `v.length()` のような形式で呼び出す。
