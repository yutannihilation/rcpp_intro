...
要素へのアクセスには `[]` または `()` を用いる。`[]` はベクトルの範囲外へのアクセスを無視するが、`()` はエラーとなる。
...
`[]` 演算子でベクトルの要素へアクセスした時の返値は、正確には `Vector`そのものではなく `Vector::Proxy` という型となっている。そのため `v[]` を、そのまま他の関数に与えるとエラーになることがある。その場合には、`as()` を用いて、目的の `Vector` 型に変換する。
