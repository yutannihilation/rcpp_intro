...
### 使用上の注意
Rのベクターは C言語で実装されているので、
ベクターのサイズを変更する際には（`push_back`, `push_front` `erase` `insert`）元のベクターの値を全てコピーする処理が発生するため効率が良くない。必要ならば、`std::vector` など標準C++のコンテナ を使用することを検討すると良いだろう。
