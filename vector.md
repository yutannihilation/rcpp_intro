...
メソッドとは、個々のオブジェクトと結びついた関数である。つう
...
R はCで実装されているのでベクターのサイズを変更する操作が苦手である。
この操作を行うと、Rのメモリ上で元のベクターの値を全てコピーする処理が発生するため効率が良くない。この問題を回避するためには、`std::vector` など標準C++のコンテナ を使用することを検討すると良いだろう。
